...
import type { PublishedLesson } from "@/types/contentRepository";

export default function LessonContent({ courseId, lessonId }: { courseId: string; lessonId: string }) {
  const repository = useLearningContentRepository();
  const [result, setResult] = useState<PublishedLesson | null | undefined>(undefined);
  const [error, setError] = useState(false);

  useEffect(() => {
    let active = true;
    repository.getPublishedLesson(courseId, lessonId).then(
      (lesson) => { if (active) setResult(lesson ?? null); },
      (reason) => {
        if (!active) return;
        if (process.env.NODE_ENV === "development") console.error("[LearnAI content] Lesson validation failed", { courseId, lessonId, reason });
        setError(true);
      }
    );
    return () => { active = false; };
  }, [courseId, lessonId, repository]);

  if (result === null) notFound();
  if (error) return <ContentMessage>Lesson content could not be loaded. Please refresh to try again.</ContentMessage>;
  if (!result) return <ContentMessage>Loading lesson...</ContentMessage>;

  const { course, module, lesson, topic, skill, practiceSkill } = result;

  return (
    <article style={styles.content}>
      <Link href={`/learn/${course.id}`} style={styles.backLink}>← {course.title}</Link>
      ...
      <section style={styles.card}>
        ...
        <ul style={styles.list}>{lesson.learningObjectives.map((objective) => <li key={objective}>{objective}</li>)}</ul>
      </section>
      ...
      {lesson.workedExamples.map((example) => (
        <section key={example.id} style={styles.exampleCard}>
          ...
          <ol style={styles.steps}>{example.steps.map((step) => <li key={step}>{step}</li>)}</ol>
      ...
      {lesson.keyPoints && lesson.keyPoints.length > 0 ? (
        <section style={styles.tipCard}>
          ...
          <ul style={styles.list}>{lesson.keyPoints.map((point) => <li key={point}>{point}</li>)}</ul>
        </section>
      ) : null}
      ...
      <section style={styles.practiceCard}>
        <div>
          ...
          <p style={styles.practiceText}>Use questions from the existing LearnAI question bank.</p>
        </div>
        <Link href={`/practice/${encodeURIComponent(practiceSkill.id)}`} style={styles.practiceButton}>Practice This Skill</Link>
      </section>
    </article>
  );
}
...
function ContentMessage({ children }: { children: React.ReactNode }) {
  return <article style={styles.content}><div style={styles.message}>{children}</div></article>;
}
