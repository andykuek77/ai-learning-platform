...
export default function CourseContent({ courseId }: { courseId: string }) {
  ...
  useEffect(() => {
    let active = true;
    repository.getPublishedCourse(courseId).then(
      (result) => { if (active) setCourse(result ?? null); },
      (reason) => {
        if (!active) return;
        if (process.env.NODE_ENV === "development") console.error("[LearnAI content] Course validation failed", { courseId, reason });
        setError(true);
      }
    );
    return () => { active = false; };
  ...
  if (error) return <ContentMessage>Course content could not be loaded. Please refresh to try again.</ContentMessage>;
  if (!course) return <ContentMessage>Loading course...</ContentMessage>;
    ...
    <section style={styles.content}>
      ...
      <div style={styles.modules}>
        {course.modules.map((courseModule, moduleIndex) => (
          <section key={courseModule.id} style={styles.moduleCard}>
            <p style={styles.moduleNumber}>MODULE {moduleIndex + 1}</p>
            <h2 style={styles.moduleTitle}>{courseModule.title}</h2>
            <p style={styles.moduleDescription}>{courseModule.description}</p>
            <div style={styles.lessons}>
              {courseModule.lessons.map((lesson, lessonIndex) => {
                const topic = getTopic(lesson.taxonomy.topicId);
                const skill = getSkill(lesson.taxonomy.skillId);
                return (
                  <article key={lesson.id} style={styles.lessonRow}>
                    ...
                    <div style={styles.lessonCopy}>
                      ...
                      <p style={styles.lessonMeta}>{topic?.label} · {skill?.label}</p>
                    </div>
                    <Link href={`/learn/${course.id}/${lesson.id}`} style={styles.lessonLink}>Start lesson</Link>
                  </article>
                );
              })}
            </div>
...
function ContentMessage({ children }: { children: React.ReactNode }) {
  return <section style={styles.content}><div style={styles.message}>{children}</div></section>;
}
